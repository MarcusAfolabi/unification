    ...
    <div class="mcontainer">
                ...
                <div class="divide-y card px-4">          
                    ...
                    <div class="flex md:space-x-6 space-x-4 md:py-5 py-3 relative">
                        <a href="{{ route('resource.show', $resource) }}" class="md:w-24 md:h-24 w-36 h-24 overflow-hidden rounded-lg relative shadow-sm">
                        ...
                        <div class="flex-1 space-y-2">
                            ...
                            <a href="{{ route('resource.show', $resource) }}" class="font-semibold block text-sm"> CEC SECRETARIAT</a>
                           <div class="flex items-center justify-between">
                                <div class="flex space-x-3 items-center text-sm md:pt-3 font-medium">
                                    <div> <a target="_blank" href="{{ asset($resource->file) }}" download="{{ asset($resource->file) }}" title="{{ ucwords($resource->name) }} "> <i class="icon-feather-download mr-2"></i>  </a></div>
                                    <div class="md:block hidden">·</div>
                                    <div> <i class="icon-feather-users mr-2"></i> {{ $resource->views }} Download</div>
                                </div>
                                <a href="{{ asset($resource->file) }}" download="{{ asset($resource->file) }}" title="{{ ucwords($resource->name) }} " class="md:flex items-center justify-center h-9 px-8 rounded-md border hidden bg-gray-100">Download Now </a>
                            </div> 
                        </div>
                    </div> 
                    @empty
