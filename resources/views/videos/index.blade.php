            ...
            <div class="divide-y -mt-3 card px-5 py-2 ">
                ...
                <div class="flex sm:flex-row flex-col sm:space-x-4 py-4 relative w-full">
                    <div class="sm:w-56 w-full h-32 overflow-hidden rounded-lg relative shadow-sm flex-shrink-0">
                        <img src="{{ asset($video->image) }}" alt="{{ $video->title }}" class="w-full h-full absolute inset-0 object-cover">
                        
                        <a href="{{ route('videos.show', $video)}}"> <img src="assets/images/icon-play.svg" class="w-12 h-12 uk-position-center" alt="{{ $video->title }}" title="{{ $video->title }}"> </a>
                    </div>
                    <div class="flex-1 relative md:mt-0 mt-4">
                        <a href="{{ route('videos.show', $video)}}" class="text-xl font-semibold leading-6">{{ $video->title }}</a>
                        ...
                        <div class="flex space-x-2 items-center text-sm pt-1">
                            ...
                            <div>·</div>
                            <div> <a href="{{ route('videos.edit', $video) }}"> <span class="icon-feather-edit "></span>
                                    Edit</a></div>
                            <div>·</div>
                            <div>
                ...
                @empty
                <p class="text-center text-opacity-75"> You currently do not have video clips!</p>
                @endforelse
            </div>
        </div>
        <div class="lg:w-1/3 w-full">
            <div uk-sticky="media @m ; offset:80 ; bottom : true" class="card">
                <div class="border-b flex items-center justify-between  p-4">
                    <div>
                        <h2 class="text-lg font-semibold">Others video</h2>
                    </div>
                </div>
                <div class="p-3">
                    @forelse ($sidevideos as $video )
                    <div class="flex items-center space-x-4 rounded-md -mx-2 p-2 hover:bg-blue-50">
                        <a href="#" href="{{ route('videos.show', $video)}}" iv class="w-12 h-12 flex-shrink-0 overflow-hidden rounded-full relative">
                            <img src="{{ asset($video->image) }}" class="absolute w-full h-full inset-0 " alt="{{ $video->title }}">
                        </a>

                        <div class="flex-1">
                            <a href="{{ route('videos.show', $video)}}" class="text-base font-semibold capitalize leading-6 line-clamp-1 mt-1">
                                {{ $video->title }}
                            </a>
                            <a href="{{ route('videos.show', $video)}}" class="w-12 h-12 uk-position-center" alt="{{ $video->title }}" title="{{ $video->title }}"> </a>
                            <a href="{{ route('videos.show', $video)}}"> <div class="text-sm text-blue-500 mt-0.5"> {{ $video->views }} Streamed</div></a>
                        </div>
                    </div>
                    @empty
                <a  href="#modal-video" uk-toggle class="border-t block text-center py-2"> Not avaliable </a>
                @endforelse
                </div>
