...
class WelcomeController extends Controller
{
    ...
    {
        $user_fellowship_posts = Post::with(['images'])->select('id', 'fellowship_id', 'title', 'slug', 'user_id', 'created_at')->orderBy('fellowship_id')->inRandomOrder()->limit(5)->get();
        $fellowship_posts = Post::with(['images'])->select('id', 'title', 'slug', 'user_id', 'created_at')->where('category', 'Fellowship')->inRandomOrder()->limit(10)->get();
        $anniversary_posts = Post::with(['images'])->select('id', 'title', 'slug', 'user_id', 'created_at')->where('category', 'Anniversary')->inRandomOrder()->limit(10)->get();
